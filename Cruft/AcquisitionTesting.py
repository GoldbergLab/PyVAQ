    ...
    try:
        ...
        for i in range(NUM_IMAGES):
            try:
                ...
                bufferSize = w * h * image_result.GetBitsPerPixel() // 8
                sa = mp.RawArray(ctypes.c_uint8, bufferSize)
                sanp = np.frombuffer(sa, dtype=ctypes.c_uint8).reshape(h, w)
                print(a.shape, a.dtype)
                np.copyto(sanp, a)
                pdb.set_trace()
                cd = image_result.GetChunkData()
                print('CounterValue:', cd.GetCounterValue())
                print('FrameID:', cd.GetFrameID())
                print('TimeStamp:', cd.GetTimestamp())
